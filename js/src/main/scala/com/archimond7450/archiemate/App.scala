...
object App {
  ...
  sealed abstract class PageWithHeaderAndFooter(override val title: String)
      extends Page(title)
  ...
  def getSplitter(using
      AuthModel,
      MobileMenuModel,
      Router[Page]
  ): SplitRender[Page, HtmlElement] =
    SplitRender[Page, HtmlElement](router.currentPageSignal)
      .collectSignal[PageWithHeaderAndFooter] { signal =>
        renderWithHeaderAndFooter(signal)
      }
      .collectSignal[Overlay] { overlayPageSignal =>
        OverlayPage.render(overlayPageSignal)
      }
      .collectSignal[NotFound] { notFoundPageSignal =>
        NotFoundPage.render(notFoundPageSignal)
      }

  def renderWithHeaderAndFooter(
      signal: Signal[PageWithHeaderAndFooter]
  )(using authModel: AuthModel, mobileMenuModel: MobileMenuModel): Div = {
    ...
    div(
      span(
        ...
        mainTag(
          cls(
            "md:container md:mx-auto md:min-w-screen-md md:max-w-screen-lg card shadow-sm p-5"
          ),
  ...
  def render()(using
      mobileMenuModel: MobileMenuModel,
      authModel: AuthModel
  ): Div = {
    checkLoginStatus()
    div(
  ...
  private def header(signal: Signal[PageWithHeaderAndFooter])(using
      mobileMenuModel: MobileMenuModel,
      authModel: AuthModel
  ): HtmlElement = {
    navTag(
            ...
            div(
              ...
              child <-- {
                authModel.stateSignal.map {
                  ...
                  case Loaded(Success(AuthModel.LoggedOut)) |
                      Loaded(Failure(_)) =>
                    navLink(Login, "Login", signal.map(_ == Login))

                  case Loaded(
                        Success(AuthModel.LoggedIn(UserResponse(_, _)))
                      ) =>
                    div(
          ...
          child <-- authModel.stateSignal.map {
            ...
            case Loaded(
                  Success(
                    AuthModel.LoggedIn(
                      UserResponse(
                        UserInfo(_, _, userDisplayName, profilePictureUrl),
                        _
                      )
                    )
                  )
  ...
  private def mobileMenuButton()(using
      mobileMenuModel: MobileMenuModel
  ): Element = {
    ...
    button(
      tpe("button"),
      cls(
        "inline-flex items-center justify-center p-2 rounded-md text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-600"
      ),
  ...
  private def footer(): Div = {
    div(
      cls(
        "w-full bg-blue-500 text-white flex-none lg:z-50 lg:border-b lg:border-slate-900/10 text-center p-2"
      ),
      div(
        span("Made with "),
        span(cls("text-red-500 dark:text-red-600"), "❤️"),
        span(
          s" using Scala ${BuildInfo.scalaVersion}, Scala.js ${BuildInfo.scalaJsVersion}, Laminar & Tailwind 3"
        )
      ),
      div(
        s"© 2022 - ${BuildInfo.buildTime.take(4)} ",
        a(

...
object DashboardConnections {
  def render(
      stream: EventStream[Status[String, Either[Throwable, Connections]]]
  ): ReactiveHtmlElement[HTMLDivElement] = div(
    child <-- stream.splitStatus(
      (resolved, resolvedSignal) =>
        resolved.output match {
          case Right(connections) =>
            val youTubeConnectionsNumber =
              connections.youtubeConnections match {
                case Nil     => "No connections"
                case List(_) => "1 connection"
                case _ =>
                  s"${connections.youtubeConnections.length} connections"
              }
            div(
              div(
                ...
                div(
                  cls("md:mr-2 md:my-2"),
                  pElement(
                    if (connections.twitchConnectionExists) "Connected"
                    else "Not connected"
                  )
                ),
                if (connections.twitchConnectionExists) {
                  ...
                  a(
                    cls(
                      "bg-[#9146FF] text-white font-bold rounded-md px-4 py-2 md:mx-2 md:my-4 hover:bg-purple-700 transition"
                    ),
                    href("/oauth/twitch/connection"),
                    "Connect"
                  )
              ...
              div(
                ...
                div(
                  cls("md:mr-2 md:my-2"),
                  pElement(
                    if (connections.kickConnectionExists) "Connected"
                    else "Not connected"
                  )
                ),
                if (connections.kickConnectionExists) {
                  a(
                    disabled(true),
                    cls(
                      "bg-red-700 text-white font-bold rounded-md px-4 py-2 md:mx-2 hover:bg-red-800 transition"
              ...
              children <-- resolvedSignal
                ...
                .split(_.channelId) {
                  (channelId, initialConnection, connectionSignal) =>
                    pElement(
                      connectionSignal
                        .map(conn => s"YouTube ${conn.channelName}")
                    )
                }
            )

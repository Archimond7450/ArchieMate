...
class TwitchUserSessionsRepositorySpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A Twitch user sessions repository" should {
    val twitchApiClientProbe = testKit.createTestProbe[TwitchApiClient.Command](TwitchApiClient.actorName)
    given twitchApiClient: ActorRef[TwitchApiClient.Command] = twitchApiClientProbe.ref
    val mediatorProbe = testKit.createTestProbe[ArchieMateMediator.Command](ArchieMateMediator.actorName)
    ...
    "return no token from token id when initially started and send nothing to twitchApiClient" in {
      val twitchUserSessionsRepository = testKit.spawn(TwitchUserSessionsRepository(), s"${TwitchUserSessionsRepository.actorName}0")
      val probe = testKit.createTestProbe[TwitchUserSessionsRepository.ReturnedTokenFromId]()

      for (tokenId <- Seq(tokenId1, tokenId2, tokenId3, wrongTokenId)) {
        twitchUserSessionsRepository ! TwitchUserSessionsRepository.GetTokenFromId(probe.ref, tokenId)
        probe.expectMessage(TwitchUserSessionsRepository.ReturnedTokenFromId(None))
      }
    ...
    "return the first token from the correct id when it is the only one added and return nothing for other tokens" in {
      val twitchUserSessionsRepository = testKit.spawn(TwitchUserSessionsRepository(), s"${TwitchUserSessionsRepository.actorName}1")
      val probe = testKit.createTestProbe[TwitchUserSessionsRepository.ReturnedTokenFromId]()

      twitchUserSessionsRepository ! TwitchUserSessionsRepository.SetToken(tokenId1, userId1, sessionId1Token)
      twitchUserSessionsRepository ! TwitchUserSessionsRepository.GetTokenFromId(probe.ref, tokenId1)
      probe.expectMessage(TwitchUserSessionsRepository.ReturnedTokenFromId(Some(sessionId1Token)))
      ...
      mediatorProbe.expectNoMessage()
      twitchApiClientProbe.expectNoMessage() // TestKit doesn't support RecoveryCompleted
    }

    "return the correct token from the correct id when there is second session added, nothing for others" in {
      val twitchUserSessionsRepository = testKit.spawn(TwitchUserSessionsRepository(), s"${TwitchUserSessionsRepository.actorName}2")
      val probe = testKit.createTestProbe[TwitchUserSessionsRepository.ReturnedTokenFromId]()

      twitchUserSessionsRepository ! TwitchUserSessionsRepository.SetToken(tokenId2, userId2, sessionId2Token)
      ...
      for (tokenId <- tokenSession.keys) {
        twitchUserSessionsRepository ! TwitchUserSessionsRepository.GetTokenFromId(probe.ref, tokenId)
        probe.expectMessage(TwitchUserSessionsRepository.ReturnedTokenFromId(tokenSession(tokenId)))
      }
    ...
    "return the correct token from the correct id when there is third session added, nothing for others" in {
      val twitchUserSessionsRepository = testKit.spawn(TwitchUserSessionsRepository(), s"${TwitchUserSessionsRepository.actorName}3")
      val probe = testKit.createTestProbe[TwitchUserSessionsRepository.ReturnedTokenFromId]()

      twitchUserSessionsRepository ! TwitchUserSessionsRepository.SetToken(tokenId3, userId3, sessionId3Token)
...
object TwitchUserSessionsRepositorySpec {
  ...
  val wrongTokenId = "0"
  val wrongUserId = "0"
  val scopes: Option[List[String]] = Some(List("channel:moderate", "chat:edit", "chat:read"))

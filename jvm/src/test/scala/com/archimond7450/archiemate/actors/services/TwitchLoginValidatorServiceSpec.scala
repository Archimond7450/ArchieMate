...
class TwitchLoginValidatorServiceSpec
  ...
  val noTwitchUserId: Option[String] = None
  val someTwitchUserid: Option[String] = Some("1234")
  ...
  "A Twitch Login Validator Service" when {
    "request uuid is not present" should {
      "respond with InvalidRequest repeatedly if such request was never created" in {
        val twitchLoginValidatorService =
          testKit.spawn(TwitchLoginValidatorService())
        val probe =
          testKit.createTestProbe[TwitchLoginValidatorService.Response]()

        val uuid = randomProvider.uuid()

        twitchLoginValidatorService ! TwitchLoginValidatorService
          .RequestSucceeded(probe.ref, uuid)
      ...
      "respond with InvalidRequest if such request was created but expired" in {
        ...
        twitchLoginValidatorService ! TwitchLoginValidatorService.NewRequest(
          probe.ref,
          noTwitchUserId
        )
        val newRequest =
          probe.expectMessageType[TwitchLoginValidatorService.NewRequestCreated]

        val manualTime = ManualTime()
        manualTime.timePasses(settings.newLoginExpirationTime + 1.second)

        twitchLoginValidatorService ! TwitchLoginValidatorService
          .RequestSucceeded(probe.ref, newRequest.uuid)
        probe.expectMessage(
          TwitchLoginValidatorService.InvalidRequest(newRequest.uuid)
        )
      }
      ...
      "respond with InvalidRequest repeatedly if two requests were created but expired" in {
        ...
        twitchLoginValidatorService ! TwitchLoginValidatorService
          .RequestSucceeded(probe.ref, firstRequest.uuid)
        probe.expectMessage(
          TwitchLoginValidatorService.InvalidRequest(firstRequest.uuid)
        )
        ...
        twitchLoginValidatorService ! TwitchLoginValidatorService
          .RequestSucceeded(probe.ref, firstRequest.uuid)
        twitchLoginValidatorService ! TwitchLoginValidatorService
          .RequestSucceeded(probe.ref, secondRequest.uuid)
        probe.expectMessage(
          TwitchLoginValidatorService.InvalidRequest(firstRequest.uuid)
        )
        probe.expectMessage(
          TwitchLoginValidatorService.InvalidRequest(secondRequest.uuid)
        )
      }
    }
    ...
    "request uuid is present" should {
      "respond with Acknowledged if RequestSucceeded is received immediately and exactly once for a valid request" in {
        ...
        twitchLoginValidatorService ! TwitchLoginValidatorService.NewRequest(
          probe.ref, noTwitchUserId
        )
        val request =
          probe.expectMessageType[TwitchLoginValidatorService.NewRequestCreated]

        val command =
          TwitchLoginValidatorService.RequestSucceeded(probe.ref, request.uuid)
        twitchLoginValidatorService ! command
        probe.expectMessage(TwitchLoginValidatorService.Acknowledged(command, noTwitchUserId))
      }
      ...
      "respond with Acknowledged if RequestSucceeded is received just in time and exactly once for a valid request" in {
        ...
        val command =
          TwitchLoginValidatorService.RequestSucceeded(probe.ref, request.uuid)
        twitchLoginValidatorService ! command
        probe.expectMessage(TwitchLoginValidatorService.Acknowledged(command, someTwitchUserid))
      }
      ...
      "respond with InvalidRequest if more than one RequestSucceeded is received for a valid request" in {
        ...
        val cmd = TwitchLoginValidatorService.RequestSucceeded(
          probe.ref,
          newRequest.uuid
        )
        twitchLoginValidatorService ! cmd
        probe.expectMessage(TwitchLoginValidatorService.Acknowledged(cmd, someTwitchUserid))

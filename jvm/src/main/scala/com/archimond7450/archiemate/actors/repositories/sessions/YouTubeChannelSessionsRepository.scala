...
object YouTubeChannelSessionsRepository {
  ...
  case class GetTokenIdsForUserId(replyTo: ActorRef[ReturnedTokenIdsForUserId], userId: String) extends Command
      ...
      override protected val commandHandler: (State, Command) => Effect[Event, State] = { (state, command) =>
        command match {
          ...
          case GetTokenIdsForUserId(replyTo, userId) =>
            ...
            replyTo ! ReturnedTokenIdsForUserId(userId, tokenIdsOption.getOrElse(Nil))
            Effect.none
        }
      }

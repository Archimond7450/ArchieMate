...
object IRCListener {
  ...
  final case class SendReplyMessage(msg: String, replyToMsgId: String)
      extends Command

  def apply(channelName: String)(using
      settings: Settings,
...
private class IRCListener(channelName: String)(using
    ctx: ActorContext[IRCListener.Command],
    settings: Settings,
    decoder: IncomingMessageDecoder,
    encoder: OutgoingMessageEncoder
) {
      ...
      Behaviors.receiveMessage {
        case Connected =>
          sendMessageWithDebug(
            webSocketClient,
            OutgoingMessages.Pass(settings.twitchIRCToken)
          )
          sendMessageWithDebug(
            webSocketClient,
            OutgoingMessages.Nick(settings.twitchIRCUsername)
          )
          sendMessageWithDebug(
            webSocketClient,
          ...
          sendMessageWithDebug(
            webSocketClient,
            OutgoingMessages.Join(List(channelName))
          )
  ...
  private def authenticating(
      webSocketClient: ActorRef[WebSocketClient.Command],
      remainingMessages: Seq[IncomingMessage] = authMessages
  )(using buffer: StashBuffer[Command]): Behavior[Command] =
    Behaviors.logMessages {
      Behaviors.receiveMessage {
        ...
        case StreamDecodedMessage(msg: NumericMessage)
            if msg == remainingMessages.head =>
        ...
        case StreamFailure(ex) =>
          ctx.log.error(
            "Connection failed when authenticating. Are the environment variables for chatbot username and twitch token configured properly?",
            ex
          )
          Behaviors.same
  ...
  private def connected(
    ...
    Behaviors.receiveMessage {
      ...
      case StreamDecodedMessage(Ping) =>
        webSocketClient ! WebSocketClient.SendText(
          encoder.encode(OutgoingMessages.Pong)
        )
        Behaviors.same
      ...
      case SendMessage(msg) =>
        webSocketClient ! WebSocketClient.SendText(
          encoder.encode(OutgoingMessages.PrivMsg(channelName, msg))
        )
        Behaviors.same
      ...
      case SendReplyMessage(msg, replyToMsgId) =>
        webSocketClient ! WebSocketClient.SendText(
          encoder.encode(
            OutgoingMessages.PrivMsg(channelName, msg, Some(replyToMsgId))
          )
        )
        Behaviors.same
      ...
      case msg =>
        ctx.log.warn(
          ...
          msg
        ) // e.g. no binary messages expected
        Behaviors.same
    }
  }
  ...
  private def sendMessageWithDebug(
      webSocketClient: ActorRef[WebSocketClient.Command],

...
object SettingsControllerHelperService {
  ...
  final case class GetConnections(
      replyTo: ActorRef[GetConnectionsResponse],
      jwt: String
  ) extends PublicCommand
  private final case class TwitchConnectionReceived(
      originalCommand: GetConnections,
      twitchUserId: String,
      tryTwitchConnected: Try[Boolean]
  ) extends Command
  private final case class KickConnectionReceived(
      twitchConnection: TwitchConnectionReceived,
      tryKickConnected: Try[Boolean]
  ) extends Command
  private final case class GetYouTubeConnections(
      twitchConnection: KickConnectionReceived,
      tokenIds: List[String],
      receivedChannels: Map[String, Try[
        YouTubeApiResponse.Response[YouTubeApiResponse.Channel]
      ]] = Map.empty
  ) extends Command
  final case class GetBasicChatbotSettings(
      replyTo: ActorRef[GetBasicChatbotSettingsResponse],
      jwt: String
  ) extends PublicCommand
  final case class GetBuiltInCommandsSettings(
      replyTo: ActorRef[GetBuiltInCommandsSettingsResponse],
      jwt: String
  ) extends PublicCommand
  final case class GetCommandsSettings(
      replyTo: ActorRef[GetCommandsSettingsResponse],
      jwt: String
  ) extends PublicCommand
  final case class GetVariablesSettings(
      replyTo: ActorRef[GetVariablesSettingsResponse],
      jwt: String
  ) extends PublicCommand
  final case class GetTimersSettings(
      replyTo: ActorRef[GetTimersSettingsResponse],
      jwt: String
  ) extends PublicCommand
  final case class GetOverlaysSettings(
      replyTo: ActorRef[GetOverlaysSettingsResponse],
      jwt: String
  ) extends PublicCommand
  final case class GetAutomaticMessagesSettings(
      replyTo: ActorRef[GetAutomaticMessagesSettingsResponse],
      jwt: String
  ) extends PublicCommand
  private final case class DecodeJWT(originalCommand: PublicCommand)
      extends Command
  private final case class PublicCommandWithUserId(
      originalCommand: PublicCommand,
      userId: String
  ) extends Command
  private final case class SendReply[T](replyTo: ActorRef[T], response: T)
      extends Command
  final case class ChangeBasicChatbotSettings(
      replyTo: ActorRef[ChangeSettingsResponse],
      jwt: String,
      settings: BasicChatbotSettings
  ) extends PublicCommand
  final case class ChangeBuiltInChatbotSettings(
      replyTo: ActorRef[ChangeSettingsResponse],
      jwt: String,
      settings: BuiltInCommandsSettings
  ) extends PublicCommand
  final case class ChangeCommandsSettings(
      replyTo: ActorRef[ChangeSettingsResponse],
      jwt: String,
      settings: CommandsSettings
  ) extends PublicCommand
  final case class ChangeVariablesSettings(
      replyTo: ActorRef[ChangeSettingsResponse],
      jwt: String,
      settings: VariablesSettings
  ) extends PublicCommand
  final case class ChangeTimersSettings(
      replyTo: ActorRef[ChangeSettingsResponse],
      jwt: String,
      settings: TimersSettings
  ) extends PublicCommand
  final case class ChangeOverlaysSettings(
      replyTo: ActorRef[ChangeSettingsResponse],
      jwt: String,
      settings: OverlaysSettings
  ) extends PublicCommand
  final case class ChangeAutomaticMessagesSettings(
      replyTo: ActorRef[ChangeSettingsResponse],
      jwt: String,
  ...
  final case class GetConnectionsOKResponse(
      tryTwitchConnected: Try[Boolean],
      tryKickConnected: Try[Boolean],
      tryYouTubeConnections: Try[List[Try[YouTubeConnection]]]
  ) extends GetConnectionsResponse
  final case class YouTubeConnection(
      channelId: String,
      channelName: String,
      channelProfileImageUrl: String
  )
  sealed trait GetBasicChatbotSettingsResponse
  final case class GetBasicChatbotSettingsOKResponse(
      trySettings: Try[BasicChatbotSettings]
  ) extends GetBasicChatbotSettingsResponse
  sealed trait GetBuiltInCommandsSettingsResponse
  final case class GetBuiltInCommandsSettingsOKResponse(
      trySettings: Try[BuiltInCommandsSettings]
  ) extends GetBuiltInCommandsSettingsResponse
  sealed trait GetCommandsSettingsResponse
  final case class GetCommandsSettingsOKResponse(
      trySettings: Try[CommandsSettings]
  ) extends GetCommandsSettingsResponse
  sealed trait GetVariablesSettingsResponse
  final case class GetVariablesSettingsOKResponse(
      trySettings: Try[VariablesSettings]
  ) extends GetVariablesSettingsResponse
  sealed trait GetTimersSettingsResponse
  final case class GetTimersSettingsOKResponse(trySettings: Try[TimersSettings])
      extends GetTimersSettingsResponse
  sealed trait GetOverlaysSettingsResponse
  final case class GetOverlaysSettingsOKResponse(
      trySettings: Try[OverlaysSettings]
  ) extends GetOverlaysSettingsResponse
  sealed trait GetAutomaticMessagesSettingsResponse
  final case class GetAutomaticMessagesSettingsOKResponse(
      trySettings: Try[AutomaticMessagesSettings]
  ) extends GetAutomaticMessagesSettingsResponse
  sealed trait ChangeSettingsResponse
  case object SettingsChanged extends ChangeSettingsResponse
  final case class SettingsFailedToChange(cause: Throwable)
      extends ChangeSettingsResponse
  ...
  private def sendErrorResponse(
      ctx: ActorContext[SettingsControllerHelperService.Command],
      cmd: PublicCommand,
      ex: Throwable
  ): SendReply[_] = cmd match {
    case GetConnections(replyTo, _) =>
      SendReply(
        replyTo,
        GetConnectionsOKResponse(Failure(ex), Failure(ex), Failure(ex))
      )
    case GetBasicChatbotSettings(replyTo, _) =>
      SendReply(replyTo, GetBasicChatbotSettingsOKResponse(Failure(ex)))
    case GetBuiltInCommandsSettings(replyTo, _) =>
      SendReply(replyTo, GetBuiltInCommandsSettingsOKResponse(Failure(ex)))
    case GetCommandsSettings(replyTo, _) =>
      SendReply(replyTo, GetCommandsSettingsOKResponse(Failure(ex)))
    case GetVariablesSettings(replyTo, _) =>
      SendReply(replyTo, GetVariablesSettingsOKResponse(Failure(ex)))
    case GetTimersSettings(replyTo, _) =>
      SendReply(replyTo, GetTimersSettingsOKResponse(Failure(ex)))
    case GetOverlaysSettings(replyTo, _) =>
      SendReply(replyTo, GetOverlaysSettingsOKResponse(Failure(ex)))
    case GetAutomaticMessagesSettings(replyTo, _) =>
      SendReply(replyTo, GetAutomaticMessagesSettingsOKResponse(Failure(ex)))
    case ChangeBasicChatbotSettings(replyTo, _, _) =>
      SendReply(replyTo, SettingsFailedToChange(ex))
    case ChangeBuiltInChatbotSettings(replyTo, _, _) =>
      SendReply(replyTo, SettingsFailedToChange(ex))
    case ChangeCommandsSettings(replyTo, _, _) =>
      SendReply(replyTo, SettingsFailedToChange(ex))
    case ChangeVariablesSettings(replyTo, _, _) =>
      SendReply(replyTo, SettingsFailedToChange(ex))
    case ChangeTimersSettings(replyTo, _, _) =>
      SendReply(replyTo, SettingsFailedToChange(ex))
    case ChangeOverlaysSettings(replyTo, _, _) =>
      SendReply(replyTo, SettingsFailedToChange(ex))
    case ChangeAutomaticMessagesSettings(replyTo, _, _) =>
      SendReply(replyTo, SettingsFailedToChange(ex))
  }
}

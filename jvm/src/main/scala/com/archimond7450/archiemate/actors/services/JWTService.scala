...
object JWTService {
  ...
  final case class GenerateJWT(
      replyTo: ActorRef[GeneratedJWT],
      userId: String,
      sessionId: String
  ) extends Command
          ...
          case DecodeJWT(replyTo, jwt) =>
            val response: DecodeJWTResponse = {
              if (JwtCirce.isValid(jwt, settings.jwtSecret, Seq(algorithm))) {
                given Clock = Clock.fixed(
                  Instant.ofEpochSecond(
                    OffsetDateTime.now(ZoneId.of("Z")).toEpochSecond
                  ),
                  ZoneId.of("Z")
                )
                ...
                JwtCirce.decode(jwt, settings.jwtSecret, Seq(algorithm)) match {
                  case Success(claims)
                      if claims.isValid && claims.issuer.contains(
                        settings.jwtIssuer
                      ) && claims.audience.nonEmpty && claims.jwtId.nonEmpty =>

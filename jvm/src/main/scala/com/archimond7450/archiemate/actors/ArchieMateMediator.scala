...
object ArchieMateMediator {
  ...
  final case class SendAutomaticMessagesSettingsRepositoryCommand(
      cmd: AutomaticMessagesSettingsRepository.Command
  ) extends Command
  final case class SendBasicChatbotSettingsRepositoryCommand(
      cmd: BasicChatbotSettingsRepository.Command
  ) extends Command
  final case class SendBuiltInCommandsSettingsRepositoryCommand(
      cmd: BuiltInCommandsSettingsRepository.Command
  ) extends Command
  final case class SendCommandsSettingsRepositoryCommand(
      cmd: CommandsSettingsRepository.Command
  ) extends Command
  final case class SendOverlaysSettingsRepositoryCommand(
      cmd: OverlaysSettingsRepository.Command
  ) extends Command
  final case class SendTimersSettingsRepositoryCommand(
      cmd: TimersSettingsRepository.Command
  ) extends Command
  ...
  final case class SendOAuthControllerHelperServiceCommand(
      cmd: OAuthControllerHelperService.Command
  ) extends Command
  final case class SendSettingsControllerHelperServiceCommand(
      cmd: SettingsControllerHelperService.Command
  ) extends Command
  ...
  final case class SendTwitchApiPaginationHandlerServiceCommand(
      cmd: TwitchApiPaginationHandlerService.Command
  ) extends Command
...
final class ArchieMateMediator(using
  ...
  case class State(
      ...
      automaticMessagesSettingsRepository: ActorRef[
        AutomaticMessagesSettingsRepository.Command
      ],
      basicChatbotSettingsRepository: ActorRef[
        BasicChatbotSettingsRepository.Command
      ],
      builtInCommandsSettingsRepository: ActorRef[
        BuiltInCommandsSettingsRepository.Command
      ],
      ...
      oauthControllerHelperService: ActorRef[
        OAuthControllerHelperService.Command
      ],
      settingsControllerHelperService: ActorRef[
        SettingsControllerHelperService.Command
      ],
  ...
  object State {
    val initial = State(
      ...
      automaticMessagesSettingsRepository = ctx.spawn(
        AutomaticMessagesSettingsRepository(),
        AutomaticMessagesSettingsRepository.actorName
      ),
      basicChatbotSettingsRepository = ctx.spawn(
        BasicChatbotSettingsRepository(),
        BasicChatbotSettingsRepository.actorName
      ),
      builtInCommandsSettingsRepository = ctx.spawn(
        BuiltInCommandsSettingsRepository(),
        BuiltInCommandsSettingsRepository.actorName
      ),
      commandsSettingsRepository = ctx.spawn(
        CommandsSettingsRepository(),
        CommandsSettingsRepository.actorName
      ),
      overlaysSettingsRepository = ctx.spawn(
        OverlaysSettingsRepository(),
        OverlaysSettingsRepository.actorName
      ),
      timersSettingsRepository = ctx
        .spawn(TimersSettingsRepository(), TimersSettingsRepository.actorName),
      variablesSettingsRepository = ctx.spawn(
        VariablesSettingsRepository(),
      ...
      oauthControllerHelperService = ctx.spawn(
        OAuthControllerHelperService(),
        OAuthControllerHelperService.actorName
      ),
      settingsControllerHelperService = ctx.spawn(
        SettingsControllerHelperService(),
        SettingsControllerHelperService.actorName
      ),
      userControllerHelperService = ctx.spawn(
        UserControllerHelperService(),

...
object OAuthControllerHelperService {
  ...
  private final case class PrepareErrorResponse(
      replyTo: ActorRef[TwitchRedirectResponse],
      ex: Throwable
  ) extends Command
  ...
  final case class CannotRedirect(message: String)
      extends TwitchRedirectResponse
  ...
  private final case class NoSuchLogin(originalRequest: TwitchCodeReceived)
      extends Command
  private final case class UnknownError(
      replyTo: ActorRef[TwitchCodeReceivedResponse],
      ex: Throwable
  ) extends Command
  private final case class GetTwitchToken(originalRequest: TwitchCodeReceived)
      extends Command
  private final case class LoginFailed(
  ...
  private final case class GetTokenUser(
      originalRequest: TwitchCodeReceived,
      token: TwitchApiResponse.GetToken
  ) extends Command
  private final case class RememberToken(
      originalRequest: TwitchCodeReceived,
      token: TwitchApiResponse.GetToken,
      tokenUser: TwitchApiResponse.GetTokenUser
  ) extends Command
  private final case class NewJWT(
      originalRequest: TwitchCodeReceived,
      jwt: String
  ) extends Command
      ...
      Behaviors.setup { ctx =>
        ...
        Behaviors.receiveAndLogMessage {
            ...
            ctx.ask[
              ...
              ref =>
                ArchieMateMediator.SendTwitchLoginValidatorServiceCommand(
                  TwitchLoginValidatorService.NewRequest(ref)
                )
            ) {
              case Success(
          ...
          case NoSuchLogin(originalRequest) =>
            ...
            originalRequest.replyTo ! Unauthorized(
          ...
          case UnknownError(replyTo, ex) =>
            ...
            replyTo ! InternalError(
              "Unknown error. Details have been logged. If the problem persists, please contact Archimond7450."
            )
            Behaviors.same
          ...
          case TwitchApiClientFailure(originalRequest, ex) =>
            ...
            originalRequest.replyTo ! InternalError(

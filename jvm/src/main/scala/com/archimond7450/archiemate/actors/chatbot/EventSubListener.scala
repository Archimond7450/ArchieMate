...
object EventSubListener {
  ...
  private final case class StreamDecodedMessage(msg: IncomingMessage)
      extends Command
...
private class EventSubListener(
  ...
  private case class RequestSubscriptionParameters(
      sessionId: String,
  ...
  private val query = Uri.Query(
    "keepalive_timeout_seconds" -> settings.twitchEventSubKeepalive.toSeconds.toString
  )

  private def initial: Behavior[Command] = {
    val webSocketClient = spawnWebSocketClient(
      settings.twitchEventSubUri.withQuery(query)
    )
    disconnected(webSocketClient)
  }

  private def disconnected(
      webSocketClient: ActorRef[WebSocketClient.Command]
  ...
  private def connected(
      remainingSubscriptions: Seq[RequestSubscriptionParameters] = Seq.empty
  )(using
      webSocketClient: ActorRef[WebSocketClient.Command]
  ): Behavior[Command] = Behaviors.receiveAndLogMessage {
    ...
    case StreamDecodedMessage(
          IncomingMessage(metadata, Payload(Some(session), _, _))
      ...
      scheduler.startTimerAtFixedRate(
        RequestSubscription,
        RequestSubscription,
        100.millis,
        100.millis
      ...
      connected(
        Seq(
          ...
          RequestSubscriptionParameters(
            ...
            "channel.raid",
            "1",
            Condition(toBroadcasterUserId = userId)
          ),
          RequestSubscriptionParameters(
            sessionId,
            "channel.raid",
            "1",
            Condition(fromBroadcasterUserId = userId)
          ),
          RequestSubscriptionParameters(
            sessionId,
            "channel.ban",
  ...
  private def operational(using
    ...
    case StreamDecodedMessage(
          IncomingMessage(
            metadata,
            Payload(Some(Session(_, _, _, Some(reconnectUrl), _)), _, _)
          )
  ...
  private def reconnecting(
      webSocketClientOld: ActorRef[WebSocketClient.Command],
      webSocketClientNew: ActorRef[WebSocketClient.Command]
  ): Behavior[Command] = Behaviors.receiveAndLogMessage {
    case StreamTextMessage(msg) =>
      decodeAndProcess(msg)
      Behaviors.same
    ...
    case StreamDecodedMessage(IncomingMessage(metadata, Payload(Some(_), _, _)))
        if metadata.messageType == "session_welcome" =>
      ctx.stop(webSocketClientOld)
      scheduler.startSingleTimer(
        Done,
        Done,
        settings.askTimeout.duration
      ) // In case the actor is stopped before the Done message is sent
  ...
  private def reconnected(using
      webSocketClient: ActorRef[WebSocketClient.Command]
  ): Behavior[Command] = Behaviors.withStash(64) { buffer =>
    Behaviors.receiveAndLogMessage {
      ...
      case Done =>
        ctx.log.debug(
          "Old WebSocket connection closed, returning to operational state"
        )
  ...
  private val textMessageTransform: TextMessage.Strict => Command = msg =>
    StreamTextMessage(msg.text)
  private val binaryMessageTransform: BinaryMessage.Strict => Command = msg =>
    StreamBinaryMessage(msg.data)
  ...
  private def requestSubscription(
      parameters: RequestSubscriptionParameters
  ): Unit = {
    mediator ! ArchieMateMediator.SendTwitchApiClientCommand(
      TwitchApiClient.CreateEventSubWebsocketSubscription(
        ctx.system.ignoreRef,
        tokenId,
        parameters.sessionId,
        parameters.subscriptionType,
        parameters.subscriptionVersion,

...
object WebSocketClient {
  ...
  private final case class InternalConnected(queue: SourceQueueWithComplete[Message]) extends Command
  ...
  private final case class IncomingText(text: TextMessage.Strict) extends Command
  private final case class IncomingBinary(bin: BinaryMessage.Strict) extends Command

  def apply[T](uri: Uri, parent: ActorRef[T], connectedNotification: () => T, textMessageTransform: TextMessage.Strict => T, binaryMessageTransform: BinaryMessage.Strict => T, doneNotification: () => T, failNotification: Throwable => T): Behavior[Command] = Behaviors.setup { ctx =>
    ...
    val incoming: Sink[Message, _] = Sink.foreach {
      case tm: TextMessage.Strict => self ! IncomingText(tm)
      case bm: BinaryMessage.Strict => self ! IncomingBinary(bm)
      case tm: TextMessage.Streamed => ctx.pipeToSelf(tm.toStrict(settings.askTimeout.duration)) {
        case Success(text) => IncomingText(text)
        case Failure(ex) => StreamToTextFailed(ex)
      }
      case bm: BinaryMessage.Streamed => bm.toStrict(settings.askTimeout.duration).onComplete {
        case Success(bin) => self ! IncomingBinary(bin)
        case Failure(ex) => StreamToBinaryFailed(ex)
      }
    }
    
    val flow = Flow.fromSinkAndSource(incoming, outgoing)
    
    ctx.pipeToSelf(Http().singleWebSocketRequest(WebSocketRequest(uri), flow)._1) {
      case Success(resp: WebSocketUpgradeResponse) if resp.response.status == StatusCodes.SwitchingProtocols =>
    ...
    waitingForQueue(parent, connectedNotification, textMessageTransform, binaryMessageTransform, doneNotification, failNotification)
  }

  private def waitingForQueue[T](parent: ActorRef[T], connectedNotification: () => T, textMessageTransform: TextMessage.Strict => T, binaryMessageTransform: BinaryMessage.Strict => T, doneNotification: () => T, failNotification: Throwable => T)(using ctx: ActorContext[Command]): Behavior[Command] = Behaviors.withStash(64) { buffer =>
    given ExecutionContext = ctx.executionContext
    ...
    Behaviors.receiveAndLogMessage {
      ...
      case InternalConnected(queue) =>
        ...
        buffer.unstashAll(active(parent, textMessageTransform, binaryMessageTransform, doneNotification, failNotification, queue))
  ...
  private def active[T](parent: ActorRef[T], textMessageTransform: TextMessage.Strict => T, binaryMessageTransform: BinaryMessage.Strict => T, doneNotification: () => T, failNotification: Throwable => T, queue: SourceQueueWithComplete[Message])(using ctx: ActorContext[Command]): Behavior[Command] = Behaviors.receiveAndLogMessage {
    case SendText(text) =>
    ...
    case StreamToBinaryFailed(ex) =>
      ctx.log.error("Failed to convert streamed binary message to strict!", ex)
      Behaviors.same
    ...
    case InternalFailed(ex: RuntimeException) if ex.getMessage.contains("WebSocket upgrade failed") =>
      ctx.log.error("Websocket upgrade failed")
      parent ! failNotification(ex)
      Behaviors.stopped

    case InternalFailed(ex: PeerClosedConnectionException) =>
      ctx.log.error("Peer closed the WebSocket connection with code {} and reason {}", ex.closeCode, ex.closeReason)
      parent ! failNotification(ex)
      Behaviors.stopped

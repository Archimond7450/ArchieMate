...
public class HostTargetEnd : Message
{
    private static readonly Regex regularExpression = new Regex(@":tmi\.twitch\.tv\sHOSTTARGET\s#(?'hosting_channel'\S+)\s:-\s(?'num_viewers'.*)");
    ...
    public HostTargetEnd(GroupCollection groups) : base(groups)
    {
